...
function showFetchingAnimation() {
  ...
  const interval = setInterval(() => {
    process.stdout.write(`\r${frames[i]} Fetching new ProPublica articles...`);
    i = (i + 1) % frames.length;
  }, 80);
  return interval;
}
...
async function processLatestArticle(isStartupPhase = false) {
  try {
    console.log('Fetching latest ProPublica articles...');
    ...
    if (previousUrls.includes(latestArticle.url)) {
    ...
    // Add URL to previousUrls before sending to prevent duplicates
    previousUrls.push(aiResult.url);
...
async function checkForNewArticles() {
  try {
    ...
    console.log('Checking for new ProPublica articles...');
    if (!articles || articles.length === 0) {
    ...
    for (const article of newUrls) {
      ...
      logNewArticle('ProPublica', article.url);
    }

    globalStats.addNewArticles('ProPublica', processedUrls.length);
    await savePreviousUrls();
    consecutiveFailures = 0;
...
async function main() {
  console.log('Starting ProPublica monitoring...');
  ...
  if (initialUrl) {
    globalStats.addNewArticles('ProPublica', 1);
    console.log(`Initial article processed: ${initialUrl}`);
  } else {
    console.log('No new initial article to process or already processed.');
  }
...
main().catch(error => {
  logError(`Fatal error in ProPublica main script: ${error.message}`);
